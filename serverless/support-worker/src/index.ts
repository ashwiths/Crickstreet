...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const validPaths = ['/', '/send-support-ticket', '/api/support/send'];

    if (request.method !== 'POST' || !validPaths.includes(url.pathname)) {
      return new Response(JSON.stringify({ success: false, error: 'Endpoint not found or method not allowed.' }), {
        status: 405,
    ...
    try {
      ...
      if (!subject || typeof subject !== 'string' || !subject.trim()) {
        return new Response(JSON.stringify({ success: false, error: 'Subject is required.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
      ...
      if (subject.length > 200) {
        return new Response(JSON.stringify({ success: false, error: 'Subject must be under 200 characters.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
      ...
      if (message.length > 5000) {
        return new Response(JSON.stringify({ success: false, error: 'Message must be under 5000 characters.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
      ...
      // 2. Secret & Recipient config (Fixed server-side support Gmail)
      const RESEND_API_KEY = env.RESEND_API_KEY;
      const SUPPORT_EMAIL = env.SUPPORT_EMAIL || 'infantashil55@gmail.com';
      const SENDER_EMAIL = env.RESEND_SENDER_EMAIL || 'onboarding@resend.dev';

      if (!RESEND_API_KEY) {
        console.error('[Cloudflare Worker] RESEND_API_KEY is missing from environment secrets.');
        return new Response(
          JSON.stringify({
            success: false,
            error: 'Server email secret (RESEND_API_KEY) is not configured.',
          }),
  ...
  <div class="card">
    ...
    <div class="body">
      ...
      ${
        screenshotUrl
          ? `
      <div class="section-title">Screenshot Attachment</div>
      <div class="field-row">
    ...
    <div class="footer">
      Delivered securely via Crickstreet Cloudflare Worker Support System.
    </div>
  </div>
</body>
</html>
      ...
      if (!resendResponse.ok) {
        console.error('[Cloudflare Worker] Resend error:', resendData);
        return new Response(
          JSON.stringify({
            success: false,
      ...
      return new Response(JSON.stringify({ success: true, emailId: resendData.id, ticketId: safeRef }), {
        status: 200,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    } catch (error: any) {
      console.error('[Cloudflare Worker] Unexpected error:', error);
      return new Response(
