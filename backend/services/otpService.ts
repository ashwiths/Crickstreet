 ...
 * storing its hash in the database.
 * 
...
export async function generateOtp(email: string): Promise<string> {
  ...
  if (!snapshot.empty) {
    // Check if the most recent OTP was created within the last 60 seconds (rate limiting)
    ...
    // 2. Mark previous unexpired OTPs as used to prevent further attempts (Delete/Invalidate)
    const batch = db.batch();
    snapshot.forEach((doc) => {
      batch.update(doc.ref, { used: true });
    });
    await batch.commit();
    console.log(`[OTP Service] Invalidated previous unused OTP records for ${email}`);
  }
  ...
  console.log(`[OTP Service] Generated and stored new OTP record for ${email}. Expires at: ${expiresAt.toLocaleTimeString()}`);
  return rawCode;
}
...
export async function verifyOtpCode(email: string, rawCode: string): Promise<boolean> {
  ...
  console.log(`[OTP Service] Verification code successfully validated for ${email}`);
  return true;
}
